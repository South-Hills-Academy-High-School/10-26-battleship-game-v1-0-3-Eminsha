...
function isPXwinner (enemyBoats: Sprite[][], hitOrMissPX: Sprite[]) {
    killCount = 0
    ...
    for (let index = 0; index <= 2; index++) {
        for (let currentBoatSprite3 of enemyBoats[index]) {
            if (grid.spriteCol(currentBoatSprite3) == grid.spriteCol(cursor) && grid.spriteRow(currentBoatSprite3) == grid.spriteRow(cursor)) {
                ...
                game.splash("" + currentPlayer + "   HIT!!" + convertToText(isPXwinner(enemyBoats, hitOrMissPX)) + " Boates destroyed!")
                return 1
            }
        }
    }
